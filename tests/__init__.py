...
with open("tests/xkcd_api_example_response_628.json") as example_628, open(
    "tests/xkcd_api_example_response_138.json"
) as example_138, open(
    "tests/xkcd_api_example_response_wrong_fields.json"
) as example_wrong:
...
def check_comic(test_case, comic, example, raw_image=True):
    ...
    """
    date = datetime.date(
        int(example["year"]), int(example["month"]), int(example["day"])
    )
    ...
    test_case.assertEqual(
        comic.explanation_url, "{}{}".format(explanation_wiki_url, example["num"])
    )
